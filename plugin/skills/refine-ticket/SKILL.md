---
name: refine-ticket
description: Turn a thin ticket into one that can be worked — read it, investigate the code, ask only questions whose answers change the outcome, then write back a clear description, file-anchored implementation plan, and executable QA plan. Use when asked to groom, refine, flesh out, scope, estimate, or make ready a ticket. Not for finding unknown problems (use qa), fixing a reported defect now (use bugfix), or implementing the ticket.
compatibility: Requires filesystem read access, a structured question mechanism or numbered chat fallback, and Abacus MCP get_ticket, update_ticket, add_comment, list_boards, list_tickets, and list_members.
---

Groom the ticket named in the current user request until someone else could
pick it up cold
and work it.
...
Call Abacus MCP `get_ticket`. It takes the key directly, and case and
zero-padding don't matter, so `cm-1` resolves. If the current request contains a
title fragment rather than a key, find it with Abacus MCP `list_boards` then
Abacus MCP `list_tickets`, and confirm which one you landed on before doing
anything else.

Read all of it: description, existing plan, existing QA plan, **every
comment**, type, swimlane, assignee, and whether it sits under a project.
Comments are where decisions already made go to hide — grooming a ticket into a plan that contradicts a
decision recorded in its own comment thread is an avoidable embarrassment. If
the ticket *is* a project, Abacus MCP `get_ticket` is the only place its
contents are listed; the board tools hide them.
...
On approval, call Abacus MCP `update_ticket` with **three separate fields**:
...
Abacus MCP `add_comment` is the right home for it, since the plan should read as the
...
- **Don't implement the ticket.** No branches, no code edits, no PRs. The
  groomed ticket is the deliverable; the `bugfix` and `ship` skills
