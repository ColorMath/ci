...
compatibility: Requires filesystem read access, host-native invocation of refine-ticket, a structured question mechanism or chat fallback, and Abacus MCP get_ticket, add_comment, list_boards, and list_tickets.
---

Plan every ticket under the initiative named in the current user request, one
at a time, until
the whole initiative is ready to pick up.

Each ticket is planned by the `refine-ticket` skill, which already knows how
...
Call Abacus MCP `get_ticket` with the initiative from the current request. It
takes the key directly. If the request contains a title fragment rather than a
key, find it with Abacus MCP `list_boards` then Abacus MCP `list_tickets` and
confirm which one you landed on.

**Check `type` is `initiative`.** If it is an ordinary ticket, this is the wrong
skill — one ticket belongs to `refine-ticket`, and say so.

**Check it has tickets.** The children are in the Abacus MCP `get_ticket`
response; the board tools hide them. If there are none:
...
- `initiative_status` is `designing` — **the tickets have not been cut yet.**
  ...
  that, point at `refine-initiative` if the features themselves still
...
For each child, call Abacus MCP `get_ticket` and note whether it is already
planned. A ticket is **planned when it carries both a `plan` and a `qa_plan`** —
the pair
...
For each ticket that needs it, in order, invoke the `refine-ticket` skill
through the host's native skill mechanism with the key **first**, followed by
the context it
...
**Verify before moving on.** Call Abacus MCP `get_ticket` on the ticket again
and confirm it now carries both a `plan` and a `qa_plan`. If it does not — the
user cut the
...
call Abacus MCP `add_comment` on the
...
- **Don't do refine-ticket's job.** No drafting plans yourself, no suppressing
  ...
  Abacus MCP `update_ticket` tool for exactly that reason.
